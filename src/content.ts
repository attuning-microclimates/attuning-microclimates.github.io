...
export const sections: Section[] = [
  ...
  },
  {
    ...
    heading: {
      zh: '可穿戴的微氣候',
      en: 'Wearable Microclimate'
    },
    paragraphs: [
      {
        zh: '展覽開幕活動中，微氣候將離開展場、走上身體。一系列「可穿戴植物淨化裝置」（Wearable Ecological Art）以能淨化空氣的室內植物為核心，讓綠意沿著人體生長——當佩戴者走動、呼吸，一座微型生態系也隨之移動。在空氣污染日益嚴重的時代，這些裝置不只是服裝，更是一種未來生活的提案：人與植物彼此依存、共同呼吸。',
        en: 'At the exhibition opening event, microclimate leaves the gallery and moves onto the body. A series of Wearable Ecological Art devices, centred on air-purifying indoor plants, allow greenery to grow along the human form — as the wearer moves and breathes, a miniature ecosystem moves with them. In an era of worsening air pollution, these devices are not merely garments but a proposal for a future way of living: humans and plants in mutual dependence, breathing together.'
      },
      {
        zh: '植物穿戴裝置使用和展場植栽球的同類植物，也是這座微氣候共同體向身體的延伸。當植物可以被穿戴、被攜帶、隨人移動，「感知微氣候」便不再侷限於展場之內：它成為一種隨身的姿態，一種與空氣、與自然重新建立關係的方式。',
        en: 'The wearable plant devices use the same species as the gallery\'s hanging planter spheres — they are an extension of this microclimatic community onto the body itself. When plants can be worn, carried, and moved with a person, Attuning to Microclimates is no longer confined within the gallery: it becomes a portable disposition, a way of re-establishing one\'s relationship with air and with nature.'
      }
    ]
  }
];
